...
import ora from 'ora';
import { apiFetch, getApiUrl, loadAuth, getAuthMethod } from '../lib/api-client.js';
...
function formatPermissionError(body: Record<string, unknown>): string {
  const required = body.required_permission as string;
  const msg = [chalk.red(`Permission denied`)];
  if (required) msg.push(chalk.yellow(`  Required: ${required}`));
  return msg.join('\n');
}
...
export function createAdminCommand() {
      ...
      try {
        ...
        if (msg.includes('403')) {
          try {
            ...
            console.error(formatPermissionError(body as Record<string, unknown>));
          } catch {
  ...
  usersCmd
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.log(`${chalk.bold('Name:')} ${(u.name as string) || '-'}`);
        ...
        console.log(`${chalk.bold('Created:')} ${u.created_at ? new Date(u.created_at as string).toLocaleString() : '-'}`);

        const workspaces = u.workspaces as Array<Record<string, unknown>> | undefined;
      ...
      try {
        ...
        const rows = [
          ...
          ...tenants.map((t: Record<string, unknown>) => [
            ...
            t.name || '-',
            (t.legal_name as string) || '-',
            t.status === 'suspended' ? chalk.red('Suspended') : chalk.green('Active'),
            t.member_count || 0,
            t.journal_count || 0,
            t.created_at ? new Date(t.created_at as string).toLocaleDateString() : '-',
          ]),
        ];
  ...
  tenantsCmd
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.log(`${chalk.bold('Legal Name:')} ${(t.legal_name as string) || '-'}`);
        console.log(`${chalk.bold('Status:')} ${t.status === 'suspended' ? chalk.red(t.status as string) : chalk.green(t.status as string)}`);
        ...
        console.log(`${chalk.bold('Created:')} ${t.created_at ? new Date(t.created_at as string).toLocaleString() : '-'}`);
      } catch (error) {
        spinner.fail('Failed');
        console.error(chalk.red(error instanceof Error ? error.message : String(error)));
        process.exit(1);
      ...
      try {
        ...
        const rows = [
          ...
          ...days.map((d: Record<string, unknown>) => [
            d.date as string,
            d.new_users as number || 0,
            d.new_workspaces as number || 0,
            d.new_entries as number || 0,
          ]),
        ];
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const rows = [
          ...
          ...tenants.map((t: Record<string, unknown>, i: number) => [
            i + 1,
  ...
  settingsCmd
    ...
    .action(async (key: string, options) => {
      ...
      try {
        ...
        console.log(chalk.bold(`\nSetting: ${key}`));
        console.log(chalk.gray('─'.repeat(50)));
        console.log(`${chalk.bold('Description:')} ${(s.description as string) || '-'}`);
  ...
  settingsCmd
    ...
    .action(async (options) => {
      ...
      try {
        const items = JSON.parse(options.items) as Array<Record<string, unknown>>;
