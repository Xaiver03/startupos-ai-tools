  ...
  importExportCmd
    ...
    .action(async (filePath: string, options) => {
      ...
      try {
        ...
        console.log(`${chalk.bold('Total:')} ${result.total as number}`);
        console.log(`${chalk.green('✓ Success:')} ${result.success as number}`);
        console.log(`${chalk.red('✗ Failed:')} ${result.failed as number}`);

        const errors = result.errors as Array<Record<string, unknown>> | undefined;
        if (errors && errors.length > 0) {
          console.log(chalk.bold('\nErrors:'));
          errors.slice(0, 10).forEach((err: Record<string, unknown>) => {
