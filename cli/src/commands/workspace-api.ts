  ...
  workspaceCmd
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const workspaces = ((data.data || data.workspaces) as Array<Record<string, unknown>>) || [];
        ...
        const rows = [
          ...
          ...(workspaces as Array<Record<string, unknown>>).map((w: Record<string, unknown>) => [
            w.id,
            (w.name as string)?.substring(0, 20) || '-',
            (w.legal_name as string)?.substring(0, 20) || '-',
            (w.taxpayer_type as string) || '-',
            (w.accounting_standard as string) || '-',
          ]),
        ];
  ...
  workspaceCmd
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        const workspace = (data.data || data) as Record<string, unknown>;
        await saveWorkspace({ id: workspace.id as string, name: workspace.name as string });
      ...
      try {
        ...
        if (displayName === 'env') {
          try {
            ...
            const ws = (data.data || data) as Record<string, unknown>;
            displayName = (ws.name as string) || displayName;
          } catch {
  ...
  workspaceCmd
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const members = (Array.isArray(data) ? data : (data.data as Array<Record<string, unknown>>) || []) as Array<Record<string, unknown>>;
        if (members.length === 0) {
        ...
        const rows = [
          ...
          ...(members as Array<Record<string, unknown>>).map((m: Record<string, unknown>) => [
            (m.user_name as string)?.substring(0, 15) || '-',
            (m.email as string)?.substring(0, 25) || '-',
            (m.role as string) || '-',
            m.joined_at ? new Date(m.joined_at as string).toLocaleDateString() : '-',
          ]),
        ];
