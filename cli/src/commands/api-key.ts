  ...
  apiKeyCmd
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const keys = (Array.isArray(data) ? data : (data.data as Array<Record<string, unknown>>) || []) as Array<Record<string, unknown>>;
        if (keys.length === 0) {
        ...
        const rows = [
          ...
          ...(keys as Array<Record<string, unknown>>).map((k: Record<string, unknown>) => [
            k.id,
            k.name as string,
            (k.keyPrefix as string) || '-',
            (k.scopes as Array<string>)?.join(', ') || '-',
            k.expiresAt ? new Date(k.expiresAt as string).toLocaleDateString() : 'Never',
  ...
  apiKeyCmd
    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (options.workspace) body.workspaceId = options.workspace;
