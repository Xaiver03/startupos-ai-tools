    ...
    .action(async () => {
      ...
      try {
        ...
        if (data.data && Array.isArray(data.data)) {
          (data.data as Array<Record<string, unknown>>).forEach((model: Record<string, unknown>) => {
            console.log(`  • ${model.id as string}`);
          });
        } else {
  ...
  aiCmd
    ...
    .action(async (promptText: string, options) => {
      ...
      try {
        ...
        console.log((choices[0].message as Record<string, unknown>).content);
