...
function assertCrud(cfg, resource, operation) {
    if (!cfg.noCrud)
        return;
    const actionsHint = cfg.actions?.length
        ? `\nAvailable actions: ${cfg.actions.join(', ')}`
        : '';
    throw new Error(`"${resource}" does not support ${operation} — it is an action-only resource. ` +
...
export function createCrudCommand() {
    ...
    crudCmd
        ...
        .option('-w, --workspace <id>', 'Workspace ID')
        .option('--json', 'Output as raw JSON')
        .action(async (resource, id, options) => {
        const cfg = getResource(resource);
    ...
    crudCmd
        ...
        if (options.search) {
            ...
            entries = entries.filter(([name, cfg]) => name.includes(q) || (cfg.label && cfg.label.includes(q)));
        }
        const rows = [
