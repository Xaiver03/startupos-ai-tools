...
export function createTaxCommand() {
    ...
    taxCmd
        ...
        if (options.from)
            params.from_date = options.from;
        if (options.to)
            params.to_date = options.to;
        if (options.status)
            params.status = options.status;
        await apiGet('/api/tax-calendar/tasks', params, 'Fetching tax calendar...', {
            json: options.json,
            onSuccess: (data) => {
    ...
    taxCmd
        ...
        .option('--json', 'Output as JSON')
        .action(async (options) => {
        await apiGet('/api/tax-calendar/rules', {}, 'Fetching tax rules...', {
            json: options.json,
            onSuccess: (data) => {
                console.log(chalk.bold('\n税务规则'));
                console.log(chalk.gray('─'.repeat(80)));
    ...
    taxCmd
        ...
        await apiGet('/api/tax-calculations', params, 'Fetching tax calculations...', {
            json: options.json,
            onSuccess: (data) => {
                console.log(chalk.bold('\n税务计算'));
                console.log(chalk.gray('─'.repeat(80)));
    ...
    taxCmd
        ...
        await apiGet('/api/tax-compliance', params, 'Checking tax compliance...', {
            json: options.json,
            onSuccess: (data) => {
                console.log(chalk.bold('\n税务合规检查'));
                console.log(chalk.gray('─'.repeat(80)));
    ...
    taxCmd
        ...
        const params = { workspace_id: options.workspace };
        if (options.formType)
            params.form_type = options.formType;
        await apiGet('/api/tax-filing-forms', params, 'Fetching tax filings...', {
            json: options.json,
            onSuccess: (data) => {
                console.log(chalk.bold('\n税务申报表'));
                console.log(chalk.gray('─'.repeat(80)));
    ...
    taxCmd
        ...
        await apiGet('/api/tax-loss-carryforward', { workspace_id: options.workspace }, 'Fetching loss carryforward...', {
            json: options.json,
            onSuccess: (data) => {
                console.log(chalk.bold('\n亏损弥补'));
                console.log(chalk.gray('─'.repeat(80)));
