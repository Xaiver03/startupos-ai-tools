...
export function registerAdminExtended(adminCmd) {
    // ═══ Roles (角色权限管理) ═══
    ...
    rolesCmd.command('list')
        ...
        try {
            ...
            const items = (data.data || data.roles) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No roles found'));
                return;
            }
            const rows = [['ID', 'Name', 'Display Name', 'Permissions'], ...items.map((r) => [
                    r.id?.substring(0, 8) + '...', r.name, r.display_name || r.name,
                    Array.isArray(r.permissions) ? r.permissions.join(', ') : String(r.permissions || '-'),
                ])];
    ...
    rolesCmd.command('permissions')
        ...
        try {
            ...
            const perms = (data.data || data.permissions) || [];
    ...
    rolesCmd.command('users')
        ...
        try {
            ...
            if (items.length === 0) {
                console.log(chalk.yellow('No users with admin roles'));
                return;
            }
            const rows = [['User ID', 'Email', 'Roles'], ...items.map((u) => [
                    String(u.user_id || u.id || '').substring(0, 12) + '...', u.email || u.user_email,
                    Array.isArray(u.roles) ? u.roles.map(r => r.name || r.display_name).join(', ') : '-',
                ])];
    ...
    rolesCmd.command('create')
        ...
        try {
            ...
            if (options.display)
                body.display_name = options.display;
            if (options.permissions)
                body.permissions = JSON.parse(options.permissions);
            if (options.desc)
            ...
            if (options.json)
                ...
                console.log(`${chalk.bold('Role:')} ${options.name}`);
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    plansCmd.command('list')
        ...
        try {
            ...
            const items = (data.data || data.plans) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No plans found'));
                return;
            }
            const rows = [['ID', 'Name', 'Slug', 'Monthly', 'Annual', 'Max Users', 'Active'], ...items.map((p) => [
                    p.id?.substring(0, 8) + '...', p.name, p.slug,
    ...
    subsCmd.command('list')
        ...
        try {
            ...
            const items = (data.data || data.subscriptions) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No subscriptions found'));
                return;
            }
            const rows = [['ID', 'Workspace ID', 'Plan', 'Status', 'Period Start', 'Period End'], ...items.map((s) => [
                    s.id?.substring(0, 8) + '...', s.workspace_id?.substring(0, 8) + '...',
    ...
    couponsCmd.command('list')
        ...
        try {
            ...
            const items = (data.data || data.coupons) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No coupons found'));
                return;
            }
            const rows = [['ID', 'Name', 'Code', 'Type', 'Value', 'Used/Total', 'Active'], ...items.map((c) => [
                    c.id?.substring(0, 8) + '...', c.name, c.code, c.type,
    ...
    couponsCmd.command('create')
        ...
        try {
            const body = { name: options.name, code: options.code, type: options.type, value: Number(options.value) };
            if (options.minAmount)
                body.min_amount_cents = Number(options.minAmount);
            if (options.totalCount)
                body.total_count = Number(options.totalCount);
            if (options.startsAt)
                body.starts_at = options.startsAt;
            ...
            if (options.json)
                ...
                console.log(`${chalk.bold('Code:')} ${options.code}`);
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    couponsCmd.command('delete')
        ...
        try {
            ...
            spinner.succeed('Coupon deleted');
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    annCmd.command('list')
        ...
        try {
            ...
            const rows = [['ID', 'Title', 'Type', 'Pinned', 'Status', 'Starts', 'Expires'], ...items.map((a) => [
                    a.id?.substring(0, 8) + '...', a.title?.substring(0, 30), a.type || 'info',
    ...
    annCmd.command('create')
        ...
        try {
            const body = { title: options.title, content: options.content, type: options.type, target: options.target, is_pinned: !!options.pinned };
            if (options.startsAt)
                body.starts_at = options.startsAt;
            if (options.expiresAt)
                body.expires_at = options.expiresAt;
            ...
            if (options.json)
                ...
                console.log(`${chalk.bold('Title:')} ${options.title}`);
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    annCmd.command('delete')
        ...
        try {
            ...
            spinner.succeed('Announcement deleted');
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    ordersCmd.command('list')
        ...
        try {
            ...
            const rows = [['ID', 'Workspace', 'Amount', 'Status', 'Plan', 'Created'], ...items.map((o) => [
                    o.id?.substring(0, 8) + '...', String(o.workspace_name || o.workspace_id || '').substring(0, 12), `¥${(Number(o.amount_cents || o.total_cents) / 100).toFixed(2)}`,
    ...
    ordersCmd.command('get')
        ...
        try {
            ...
            console.log(`${chalk.bold('Created:')} ${o.created_at ? new Date(o.created_at).toLocaleString() : '-'}`);
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    kbCmd.command('list')
        ...
        try {
            ...
            if (items.length === 0) {
                console.log(chalk.yellow('No KB entries found'));
                return;
            }
            const rows = [['ID', 'Category', 'Question', 'Published', 'Sort'], ...items.map((e) => [
                    e.id?.substring(0, 8) + '...', e.category, e.question?.substring(0, 40),
    ...
    kbCmd.command('create')
        ...
        try {
            const body = { category: options.category, question: options.question, answer: options.answer, sort_order: Number(options.sort), is_published: !!options.published };
            if (options.legalBasis)
                body.legal_basis = options.legalBasis;
            if (options.tags)
    ...
    kbCmd.command('delete')
        ...
        try {
            await apiFetch(`/api/admin/compliance-kb/${id}`, { method: 'DELETE' });
            spinner.succeed('KB entry deleted');
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    trCmd.command('list')
        ...
        try {
            ...
            const items = (data.data || data.rules) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No tax rules found'));
                return;
            }
            const rows = [['ID', 'Name', 'Type', 'Priority', 'Active', 'Effective From'], ...items.map((r) => [
                    r.id?.substring(0, 8) + '...', r.name, r.rule_type, r.priority || 0,
    ...
    revCmd.command('metrics')
        ...
        try {
            ...
            console.log(chalk.gray('─'.repeat(40)));
            if (m.mrr)
                console.log(`${chalk.bold('MRR:')} ¥${Number(m.mrr).toLocaleString()}`);
            if (m.arr)
                console.log(`${chalk.bold('ARR:')} ¥${Number(m.arr).toLocaleString()}`);
            if (m.churn_rate)
                console.log(`${chalk.bold('Churn Rate:')} ${m.churn_rate}`);
            if (m.total_revenue)
                console.log(`${chalk.bold('Total Revenue:')} ¥${Number(m.total_revenue).toLocaleString()}`);
    ...
    revCmd.command('usage')
        ...
        try {
            ...
            const u = (data.data || data.usage || data);
            console.log(chalk.bold('\nPlatform Usage'));
            console.log(chalk.gray('─'.repeat(40)));
