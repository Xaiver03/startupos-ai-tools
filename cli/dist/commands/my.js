...
export function createMyCommand() {
    ...
    myCmd
        ...
        try {
            ...
            const items = (data.payslips || data.data) || [];
            if (items.length === 0) {
                console.log(chalk.yellow('No payslips found'));
                return;
            }
            const rows = [
    ...
    myCmd
        ...
        try {
            ...
            const p = (data.data || data.payslip || data);
            console.log(chalk.bold(`\nPayslip — ${period}`));
            ...
            if (p.remark)
                console.log(`${chalk.bold('Remark:')} ${p.remark}`);
        }
        catch (error) {
            spinner.fail('Failed');
            console.error(chalk.red(error instanceof Error ? error.message : String(error)));
    ...
    myCmd
        ...
        try {
            ...
            const items = (data.claims || data.data) || [];
            if (items.length === 0) {
            ...
            const rows = [
                ...
                ...items.map((c) => [
                    c.id?.substring(0, 8) + '...', c.claim_number || '-',
                    c.title?.substring(0, 25), `¥${Number(c.total_amount || 0).toLocaleString()}`,
                    c.status, c.submitted_at ? new Date(c.submitted_at).toLocaleDateString() : '-',
                ]),
            ];
            console.log(chalk.bold('\nMy Expense Claims'));
            console.log(table(rows));
    ...
    myCmd
        ...
        try {
            ...
            if (c.reject_reason)
                console.log(`${chalk.bold('Reject Reason:')} ${chalk.red(String(c.reject_reason))}`);
            const items = c.items || [];
            if (items.length > 0) {
                console.log(`\n${chalk.bold('Items:')}`);
                items.forEach((item) => {
