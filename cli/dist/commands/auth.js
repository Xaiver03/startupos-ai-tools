...
import ora from 'ora';
import { loadAuth, saveAuth, clearAuth, getApiUrl } from '../lib/api-client.js';
    ...
    authCmd
        ...
        if (options.token) {
            ...
            try {
                ...
                await saveAuth({
                    method: 'password',
                    userId: authData.user?.id || options.email,
                    email: options.email,
                ...
                if (response.ok) {
                    ...
                    if (perms.is_super_admin) {
                        ...
                        if (permissions.length > 0) {
                            permissions.slice(0, 10).forEach((p) => console.log(chalk.gray(`  - ${p}`)));
                            if (permissions.length > 10)
                                console.log(chalk.gray(`  ... and ${permissions.length - 10} more`));
                        }
                    }
                }
            }
