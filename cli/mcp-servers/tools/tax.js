export function createTaxTools(client) {
    return {
        get_tax_calendar_tasks: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/tax-calendar/tasks?${params.toString()}`);
        ...
        get_tax_calendar_rules: {
            ...
            handler: async () => {
                const data = await client.apiFetch('/api/tax-calendar/rules');
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        get_tax_calculations: {
            ...
            handler: async (args) => {
                ...
                params.append('workspace_id', client.getWorkspaceId());
                if (args.tax_type)
                    params.append('tax_type', args.tax_type);
                params.append('limit', String(args.limit || 20));
                const data = await client.apiFetch(`/api/tax-calculations?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        get_tax_filing_forms: {
            ...
            handler: async (args) => {
                ...
                params.append('workspace_id', client.getWorkspaceId());
                if (args.form_type)
                    params.append('form_type', args.form_type);
                if (args.status)
                    params.append('status', args.status);
                const data = await client.apiFetch(`/api/tax-filing-forms?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
    };
