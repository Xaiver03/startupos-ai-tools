export function createAccountingTools(client) {
    return {
        list_journal_entries: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/journal-entries?${params.toString()}`);
        ...
        get_journal_entry: {
            ...
            handler: async (args) => {
                const data = await client.apiFetch(`/api/journal-entries/${args.id}`);
                return { content: [{ type: 'text', text: JSON.stringify(data.data, null, 2) }] };
            },
        },
        list_accounts: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/accounts?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data.data, null, 2) }] };
            },
        },
        get_account_balance: {
            ...
            handler: async (args) => {
                ...
                if (args.end_date)
                    params.append('end_date', args.end_date);
                const data = await client.apiFetch(`/api/accounts/balance?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data.data, null, 2) }] };
            },
        },
        create_journal_entry: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/journal-entries`, {
                    method: 'POST',
                    body: JSON.stringify({
