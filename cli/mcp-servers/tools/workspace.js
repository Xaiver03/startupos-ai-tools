export function createWorkspaceTools(client) {
    return {
        list_workspaces: {
            ...
            handler: async () => {
                const data = await client.apiFetch('/api/workspaces');
        ...
        switch_workspace: {
            ...
            handler: async (args) => {
                const data = await client.apiFetch(`/api/workspaces/${args.workspace_id}`);
        ...
        get_workspace_settings: {
            ...
            handler: async () => {
                ...
                const data = await client.apiFetch(`/api/workspaces/${workspaceId}/settings`);
                return { content: [{ type: 'text', text: JSON.stringify(data.data, null, 2) }] };
            },
        },
    };
