export function createBankTools(client) {
    return {
        list_bank_accounts: {
            ...
            handler: async () => {
                ...
                const data = await client.apiFetch(`/api/bank-accounts?${params.toString()}`);
        ...
        create_bank_account: {
            ...
            handler: async (args) => {
                ...
                if (args.opening_balance)
                    body.opening_balance = args.opening_balance;
                const data = await client.apiFetch('/api/bank-accounts', {
                    method: 'POST',
        ...
        list_bank_transactions: {
            ...
            handler: async (args) => {
                ...
                if (args.bank_account_id)
                    params.append('bank_account_id', args.bank_account_id);
                if (args.start_date)
                    params.append('start_date', args.start_date);
                if (args.end_date)
                    params.append('end_date', args.end_date);
                params.append('limit', String(args.limit || 50));
                const data = await client.apiFetch(`/api/bank-transactions?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        import_bank_transactions: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch('/api/bank-transactions/import', {
                    method: 'POST',
                    body: JSON.stringify(body),
                });
                return {
        ...
        list_reconciliation_records: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/reconciliation-records?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
    };
