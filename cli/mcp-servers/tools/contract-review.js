export function createContractReviewTools(client) {
    return {
        review_contract_text: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch('/api/contract-reviews/text', {
                    method: 'POST',
        ...
        get_contract_review: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch(`/api/contract-reviews/${args.review_id}`);
                return {
                    content: [{ type: 'text', text: JSON.stringify(response, null, 2) }],
                };
            },
        ...
        list_contract_reviews: {
            ...
            handler: async (args) => {
                ...
                const response = await client.apiFetch(`/api/contract-reviews?${params}`);
                return {
                    content: [{ type: 'text', text: JSON.stringify(response, null, 2) }],
                };
            },
        ...
        ask_contract_question: {
            ...
            handler: async (args) => {
                ...
                const response = await client.apiFetch(`/api/contract-reviews/${review_id}/ask`, {
                    method: 'POST',
