 ...
 * @returns 统一后的交易记录
 */
export async function unifyBankStatements(files) {
 ...
 * 解析工商银行流水
 */
function parseICBCStatement(file) {
    const transactions = [];
    try {
 ...
 * 解析招商银行流水
 */
function parseCMBStatement(file) {
    const transactions = [];
    try {
        const workbook = XLSX.read(file.content, { type: 'buffer' });
        const sheet = workbook.Sheets[workbook.SheetNames[0]];
 ...
 * 解析支付宝流水
 */
function parseAlipayStatement(file) {
    const transactions = [];
    try {
        const workbook = XLSX.read(file.content, { type: 'buffer' });
        const sheet = workbook.Sheets[workbook.SheetNames[0]];
...
//# sourceMappingURL=bank-statements.js.map