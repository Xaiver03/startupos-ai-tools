export function createExpenseTools(client) {
    return {
        list_expense_claims: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/expense-claims?${params.toString()}`);
        ...
        create_expense_claim: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch('/api/expense-claims', {
                    method: 'POST',
        ...
        approve_expense_claim: {
            ...
            handler: async (args) => {
                const data = await client.apiFetch(`/api/expense-claims/${args.claim_id}/approve`, { method: 'POST' });
                return {
                    content: [{
                            type: 'text',
    ...
    return {
        list_departments: {
            ...
            handler: async () => {
                const params = new URLSearchParams();
                params.append('workspace_id', client.getWorkspaceId());
                const data = await client.apiFetch(`/api/departments?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        create_department: {
            ...
            handler: async (args) => {
                ...
                if (args.manager_id)
                    body.manager_id = args.manager_id;
                const data = await client.apiFetch('/api/departments', {
                    method: 'POST',
                    body: JSON.stringify(body),
                });
                return {
        ...
        list_projects: {
            ...
            handler: async (args) => {
                ...
                if (args.status)
                    params.append('status', args.status);
                const data = await client.apiFetch(`/api/projects?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        create_project: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch('/api/projects', {
                    method: 'POST',
                    body: JSON.stringify(body),
                });
                return {
