 ...
 * @returns 发票信息
 */
export async function parseInvoiceXML(xmlContent) {
    try {
 ...
 * @returns 检测结果
 */
export async function detectRedInvoices(xmlFiles) {
    const blueInvoices = [];
    const redInvoices = [];
    // 解析所有 XML 文件
    ...
    for (const redInvoice of redInvoices) {
        if (redInvoice.originalInvoice) {
            const originalBlue = blueInvoices.find(b => b.invoiceNumber === redInvoice.originalInvoice ||
 ...
 * @returns 是否为红字发票
 */
export async function isRedInvoice(xmlContent) {
    try {
...
//# sourceMappingURL=red-invoice-detector.js.map