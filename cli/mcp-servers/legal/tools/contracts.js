export function createContractTools(client) {
    return {
        list_contracts: {
            ...
            handler: async (args) => {
                ...
                if (args.contract_type)
                    params.append('contract_type', args.contract_type);
                const response = await client.apiFetch(`/api/contracts?${params}`);
                return {
        ...
        get_contract: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch(`/api/contracts/${args.contract_id}`);
                return {
                    content: [{ type: 'text', text: JSON.stringify(response, null, 2) }],
                };
            },
        ...
        create_contract: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch('/api/contracts', {
                    method: 'POST',
        ...
        generate_contract: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch('/api/contracts/generate', {
                    method: 'POST',
                    body: JSON.stringify(args),
                });
                return {
        ...
        update_contract: {
            ...
            handler: async (args) => {
                ...
                const response = await client.apiFetch(`/api/contracts/${contract_id}`, {
                    method: 'PATCH',
