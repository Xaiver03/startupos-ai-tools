/**
 * AI Chat engine for the Startup OS Chatbot.
...
export async function chatWithAI(ctx: ChatContext): Promise<string> {
...
function formatCommandResult(message: string, result: any): string {
  ...
  if (Array.isArray(result)) {
    ...
    result.slice(0, 5).forEach((item: any, i: number) => {
      if (item.name) {
...
function formatFinancialStatement(report: any): string {
  return `📊 财务报表：
- 营业收入: ${report.revenue || '-'}
...
function generateFallbackResponse(message: string): string {
  ...
  if (msg.includes('帮助') || msg.includes('help') || msg.includes('功能')) {
    return 'Startup OS Chatbot 支持以下功能：\n1. 会计：查询凭证、生成报表、科目管理\n2. 税务：税务日历、合规检查、申报表\n3. 银行：账户管理、流水查询、对账\n4. 发票：增值税发票、往来单位\n5. 人力资源：员工管理、薪酬、劳动合同\n6. 报销：报销单审批\n7. 法务：合同管理、审查、催款函\n8. AI 记账：智能记账、发票OCR\n\n请告诉我你需要什么帮助？';
  }
