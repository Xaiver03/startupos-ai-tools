...
async function apiFetch(path, options = {}) {
    ...
    if (!token)
        throw new Error('SSOS_API_KEY environment variable required');
    const headers = {
...
function buildParams(args, workspaceId) {
    const params = new URLSearchParams();
    if (workspaceId)
    ...
    if (args.includes('-s')) {
        ...
        if (startMatch)
            params.append('start_date', startMatch[1]);
    }
    if (args.includes('-e')) {
        const endMatch = args.match(/-e\s+(\d{4}-\d{2}-\d{2})/);
        if (endMatch)
            params.append('end_date', endMatch[1]);
    }
    if (args.includes('-l')) {
...
export async function executeCommand(command, workspaceId) {
