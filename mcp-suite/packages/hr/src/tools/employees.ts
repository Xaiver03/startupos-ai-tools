...
export function createEmployeeTools(client: SSOSClient) {
  return {
    list_employees: {
      ...
      handler: async (args: ListEmployeesInput) => {
    ...
    get_employee: {
      ...
      handler: async (args: GetEmployeeInput) => {
        const data: APIResponse = await client.apiFetch(
          `/api/employees/${args.employee_id}`
        );
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
      },
    },
    ...
    create_employee: {
      ...
      handler: async (args: CreateEmployeeInput) => {
        const body: Record<string, unknown> = {
          workspace_id: client.getWorkspaceId(),
          name: args.name,
          hire_date: args.hire_date,
        };
        if (args.id_card_no) body.id_card_no = args.id_card_no;
        if (args.department) body.department = args.department;
        if (args.position) body.position = args.position;
        if (args.monthly_special_deduction) body.monthly_special_deduction = args.monthly_special_deduction;
        if (args.status) body.status = args.status;
        if (args.remark) body.remark = args.remark;

        const data: APIResponse = await client.apiFetch('/api/employees', {
          method: 'POST',
    ...
    update_employee: {
      ...
      handler: async (args: UpdateEmployeeInput) => {
        const { employee_id, ...updates } = args;

        const data: APIResponse = await client.apiFetch(`/api/employees/${employee_id}`, {
          method: 'PATCH',
    ...
    delete_employee: {
      ...
      handler: async (args: DeleteEmployeeInput) => {
        const data: APIResponse = await client.apiFetch(`/api/employees/${args.employee_id}`, {
          method: 'DELETE',
        });
