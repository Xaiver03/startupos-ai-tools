...
export function createLaborContractTools(client: SSOSClient) {
  return {
    list_labor_contracts: {
      ...
      handler: async (args: ListLaborContractsInput) => {
        ...
        const data: APIResponse = await client.apiFetch(
          `/api/labor-contracts?${params.toString()}`
        );
    ...
    create_labor_contract: {
      ...
      handler: async (args: CreateLaborContractInput) => {
        ...
        if (args.probation_months) body.probation_months = args.probation_months;
        if (args.base_salary) body.base_salary = args.base_salary;
        if (args.remark) body.remark = args.remark;

        const data: APIResponse = await client.apiFetch('/api/labor-contracts', {
          method: 'POST',
