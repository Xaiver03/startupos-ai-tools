export function createDemandLetterTools(client) {
    return {
        list_demand_letters: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch('/api/demand-letters');
                return {
        ...
        generate_demand_letter: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch('/api/demand-letters/generate', {
                    method: 'POST',
        ...
        get_legal_path_recommendation: {
            ...
            handler: async (args) => {
                const response = await client.apiFetch(`/api/demand-letters/legal-path?amount=${args.amount}`);
                return {
                    content: [{ type: 'text', text: JSON.stringify(response, null, 2) }],
                };
            },
