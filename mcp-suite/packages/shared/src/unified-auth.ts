...
import type { SSOSConfig, APIResponse } from './types.js';
import type { AuthMethod, AuthConfig, SavedAuth } from './auth-types.js';
...
export class UnifiedAuthManager {
  ...
  private currentAuth: SavedAuth | null = null;
  ...
  private async authFlow(): Promise<void> {
    console.error('\n🔐 SSOS Authentication\n');
    ...
    const { method } = await inquirer.prompt<{ method: AuthMethod }>([
      {
        type: 'list',
        name: 'method',
  ...
  private async promptPassword(): Promise<void> {
  ...
  private async loginWithPassword(email: string, password: string): Promise<void> {
  ...
  private async oauthFlow(): Promise<void> {
  ...
  private async validateAuth(): Promise<void> {
    if (!this.currentAuth) throw new Error('No auth');
  ...
  private async loadAuth(): Promise<SavedAuth | null> {
    try {
  ...
  private async saveAuth(auth: SavedAuth): Promise<void> {
