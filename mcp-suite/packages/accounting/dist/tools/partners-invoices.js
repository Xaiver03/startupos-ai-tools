    ...
    return {
        list_vat_invoices: {
            ...
            handler: async (args) => {
                ...
                if (args.invoice_type)
                    params.append('invoice_type', args.invoice_type);
                if (args.start_date)
                    params.append('start_date', args.start_date);
                if (args.end_date)
                    params.append('end_date', args.end_date);
                params.append('limit', String(args.limit || 50));
                const data = await client.apiFetch(`/api/business-vat-invoices?${params.toString()}`);
        ...
        create_vat_invoice: {
            ...
            handler: async (args) => {
                ...
                if (args.seller_name)
                    body.seller_name = args.seller_name;
                if (args.buyer_name)
                    body.buyer_name = args.buyer_name;
                if (args.amount)
                    body.amount = args.amount;
                if (args.tax_amount)
                    body.tax_amount = args.tax_amount;
                if (args.tax_rate)
                    body.tax_rate = args.tax_rate;
                const data = await client.apiFetch('/api/business-vat-invoices', {
                    method: 'POST',
...
export function createPartnerTools(client) {
    return {
        list_partners: {
            ...
            handler: async (args) => {
                ...
                if (args.partner_type)
                    params.append('partner_type', args.partner_type);
                if (args.search)
                    params.append('search', args.search);
                const data = await client.apiFetch(`/api/partners?${params.toString()}`);
                return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
            },
        },
        create_partner: {
            ...
            handler: async (args) => {
                ...
                if (args.tax_number)
                    body.tax_number = args.tax_number;
                if (args.contact_person)
                    body.contact_person = args.contact_person;
                if (args.phone)
                    body.phone = args.phone;
                if (args.address)
                    body.address = args.address;
                if (args.bank_name)
                    body.bank_name = args.bank_name;
                if (args.bank_account)
                    body.bank_account = args.bank_account;
                const data = await client.apiFetch('/api/partners', {
                    method: 'POST',
                    body: JSON.stringify(body),
                });
                return {
        ...
        update_partner: {
            ...
            handler: async (args) => {
                ...
                const data = await client.apiFetch(`/api/partners/${partner_id}`, {
                    method: 'PATCH',
