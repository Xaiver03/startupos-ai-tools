...
export function reconcileInvoices(
  ...
  matchMethods: MatchMethod[] = ['exact', 'fuzzy', 'keyword', 'split']
  ...
  for (const [customerName, customerData] of customerInvoices.entries()) {
    ...
    // 使用多重匹配查找收款
    const matchResult = multiMatchPayment(
      customerName,
      incomeTransactions,
      totalAmount,
      matchMethods
 ...
 * 按客户汇总发票
 */
function groupInvoicesByCustomer(
  invoices: Invoice[]
): Map<string, { invoices: Invoice[]; totalAmount: number }> {
  const groups = new Map<string, { invoices: Invoice[]; totalAmount: number }>();
 ...
 * 使用多种方法查找客户的所有收款记录
 */
function multiMatchPayment(
  customerName: string,
  transactions: BankTransaction[],
  expectedAmount: number,
  methods: MatchMethod[]
): { totalReceived: number; transactions: MatchedTransaction[] } {
  ...
  if (methods.includes('split') && matched.length === 0) {
    const splitMatch = findSplitPayments(
      expectedAmount,
      transactions.filter((_, i) => !usedTransactionIndices.has(i))
    );
