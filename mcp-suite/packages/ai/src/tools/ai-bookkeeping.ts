  ...
  return {
    ai_bookkeeping: {
      ...
      handler: async (args: AIBookkeepingInput) => {
        ...
        if (args.ocr_data) body.ocr_data = args.ocr_data;
        if (args.input_mode) body.input_mode = args.input_mode;
        if (args.conversation_id) body.conversation_id = args.conversation_id;

        const data: APIResponse = await client.apiFetch('/api/ai/bookkeeping', {
          method: 'POST',
    ...
    list_ai_conversations: {
      ...
      handler: async (args: ListAIConversationsInput) => {
        ...
        const data: APIResponse = await client.apiFetch(
          `/api/ai/conversations?${params.toString()}`
        );
    ...
    ocr_invoice: {
      ...
      handler: async (args: OCRInvoiceInput) => {
        const body: Record<string, unknown> = {
          workspace_id: client.getWorkspaceId(),
          document_type: args.document_type,
        };
        if (args.file_url) body.file_url = args.file_url;
        if (args.image_url) body.image_url = args.image_url;

        const data: APIResponse = await client.apiFetch('/api/ocr', {
          method: 'POST',
          body: JSON.stringify(body),
        });
    ...
    ask_compliance_question: {
      ...
      handler: async (args: AskComplianceQuestionInput) => {
        const body = {
          workspace_id: client.getWorkspaceId(),
          question: args.question,
        };

        const data: APIResponse = await client.apiFetch('/api/compliance-qa', {
          method: 'POST',
          body: JSON.stringify(body),
        });
